...
LOADER=';document.addEventListener("DOMContentLoaded",function(){if(document.getElementById("coordenadas")&&!document.getElementById("mka-geocodificacao-js")){var s=document.createElement("script");s.id="mka-geocodificacao-js";s.src="addons/geocodificacao/geocodificacao.js?v=2.8.0";document.head.appendChild(s);}});'
...
if grep -q 'mka-geocodificacao-js' "$MAIN_JS"; then
    sed -i -E 's#geocodificacao\.js\?v=[0-9.]+#geocodificacao.js?v=2.8.0#g' "$MAIN_JS"
else
    printf '\n%s\n' "$LOADER" >> "$MAIN_JS"
fi
...
grep -q 'geocodificacao.js?v=2.8.0' "$MAIN_JS"
